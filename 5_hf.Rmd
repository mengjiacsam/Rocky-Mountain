...
```{r}
m <- length(top100_unique$acousticness)
...
data_range_sp <- data.table(x = top100_unique$acousticness, y = top100_unique$rank)

p_sp <- ggplot(data_train_sp, aes(acousticness, rank)) +
    geom_point()  +
    theme_minimal()
...
for (i in k) {
    if (i == 0) {
        ...
        model_sp <- lm(top100_unique$rank ~ poly(top100_unique$acousticness, i, raw = TRUE), data = data_train_sp)
    }
...
p_sp +
    geom_line(data = data_range_sp_long, aes(x, value)) +
    geom_point(data = data_test_sp, aes(acousticness, rank), color = "red") +
